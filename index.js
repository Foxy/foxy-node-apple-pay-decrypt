...
class PaymentToken {
  ...
  merchantId (cert) {
    try {
      const info = x509.Certificate.fromPEM(cert);
      const oid = info.extensions.find(extension => extension.oid === MERCHANT_ID_FIELD_OID);
      return oid.value.toString().substring(2);
    } catch (e) {
